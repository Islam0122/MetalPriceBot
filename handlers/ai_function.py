...
def sent_prompt_and_get_response(msg: str, data):
    ...
    if access_token:
        ...
        prompt = (
            f"Ты MetalPriceBot, ты занимаешься продажей металлов. Пользователь хочет купить: {msg}. "
            "Подбери подходящих поставщиков из следующего списка.\n"
            "Ответ должен быть кратким. Укажи только тех поставщиков, которые могут предоставить этот товар. "
            "Для каждого подходящего поставщика укажи его название, краткое описание (если возможно) и ссылку на сайт. "
            "Пример: 'Поставщик 1: описание, ссылка'. Ты должен общаться с пользователем.\n"
        )
